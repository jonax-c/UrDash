...
def _requirements_for_mode(mode: str) -> list[str]:
    if mode in {"custom_card", "custom_dashboard"}:
        return [
            ...
            "First decide the card's job: what the user must understand at a glance, what actions should be available, and what secondary context supports those actions.",
            "Then choose card types and layout to serve those functions. Do not choose card types only for decoration.",
            ...
            "Use weather for weather entities or weather-focused requests, security for locks/doors/motion/alarm, energy for power/energy sensors, and room for room-level control cards.",
            ...
            "Prefer one primary functional card plus one or two compact support cards. Avoid splitting one simple request into many low-value cards.",
            "Prefer card types climate, weather, security, energy, room, scene, metric, control, and timeline according to user intent.",
            "For climate or air conditioner requests, use a climate card as the primary card, with the climate entity in entity_ids.",
            "For climate cards, avoid hero-only output; include useful controls and a compact status/target-temperature presentation.",
            "For sensor summary cards, use metric or energy cards and include only the most relevant sensors.",
            "For room control cards, use room or control cards and include the room's controllable lights, climate, covers, switches, and key sensors.",
            "For security cards, include useful lock/door/window/alarm state and actionable controls when available.",
